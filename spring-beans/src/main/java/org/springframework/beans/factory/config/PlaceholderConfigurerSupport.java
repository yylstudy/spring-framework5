...
public abstract class PlaceholderConfigurerSupport extends PropertyResourceConfigurer
	...
	protected void doProcessProperties(ConfigurableListableBeanFactory beanFactoryToProcess,
		...
		//创建一个BeanDefinition访问器
		BeanDefinitionVisitor visitor = new BeanDefinitionVisitor(valueResolver);
		String[] beanNames = beanFactoryToProcess.getBeanDefinitionNames();
		for (String curName : beanNames) {
			// Check that we're not parsing our own bean definition,
			// to avoid failing on unresolvable placeholders in properties file locations.
			//beanName是实现BeanNameAware接口执行setBeanName方法赋值的
			//这里是如果beanName不是propertySourcesPlaceholderConfigurer
			if (!(curName.equals(this.beanName) && beanFactoryToProcess.equals(this.beanFactory))) {
				BeanDefinition bd = beanFactoryToProcess.getBeanDefinition(curName);
				try {
					//访问器去访问BeanDefinitioon，这里主要是解析容器中BeanDefinition的各个值
		...
		//向embeddedValueResolvers中添加字符串表达式，这个主要是用于后面处理@Value注解的
