...
public class SimpleInstantiationStrategy implements InstantiationStrategy {
	 ...
	 * @param owner
	 * @param factoryBean @Bean注解所在的bean 如果是静态方法，那么这个factoryBean就为空
	 * @param factoryMethod 使用的方法
	 * @param args 使用的参数
	 * @return
	 */
	@Override
	public Object instantiate(RootBeanDefinition bd, @Nullable String beanName, BeanFactory owner,
		...
		try {
			...
			try {
				//将当前@Bean方法赋值给当前线程，这个变量在 若是full模式（也就是@Configuration注解修饰的@Bean方法的类）中的代理方法中
				//有使用到
				currentlyInvokedFactoryMethod.set(factoryMethod);
				//静态方法时，factoryBean为空这是正确的
