...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * bean的class类型（或者未反射的beanClassName）
	 */
	@Nullable
	private volatile Object beanClass;
	/**
	 * 当前bean的作用域，singleton、prototype
	 */
	@Nullable
	...
	/**
	 * 自动装配的类型，这个是处理自动注入的，如果一个bean配置了这个属性
	 * 那么这个bean的属性都会尝试自动注入，无需配置依赖注入的注解或者 property
	 *
	 * @Bean 注解这个值是 构造自动注入 AUTOWIRE_CONSTRUCTOR ，所以@Bean的方法参数都是自动注入的
	 */
	private int autowireMode = AUTOWIRE_NO;

	private int dependencyCheck = DEPENDENCY_CHECK_NONE;
	/**
	 * depends-on属性，depends-on表示依赖这个bean，所以其依赖的bean需要先完成初始化
	 */
	@Nullable
	...
	/**
	 * primary属性，这个在@Autowired时出现多个候选者时，选择带有@Primary注解或者primary属性的候选者
	 */
	...
	/**
	 * bean标签factory-bean属性
	 * @Bean 注解所在类的beanName
	 */
	@Nullable
	private String factoryBeanName;
	/**
	 * 工厂方法名称 bean标签的factory-method属性
	 * @Bean 注解所在类的方法名
	 */
	@Nullable
