...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	/**
	 * 获取事务状态对象
	 * @Param definition 事务属性TransactionAttribute
	 */
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		//是否已存在事务，判断依据是线程变量变量中是否存在ConnectionHolder
	...
	/**
	 * 已存在事务
	 * @param definition 事务属性 RuleBasedTransactionAttribute
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
			...
			//挂起当前事务，保存被挂起的事务信息 SuspendedResourcesHolder
			Object suspendedResources = suspend(transaction);
			boolean newSynchronization = (getTransactionSynchronization() == SYNCHRONIZATION_ALWAYS);
			//创建一个新的事务状态，这里注意的是 事务对象为空，不是新建事务，以及挂起的事务资源 suspendedResources
	 ...
	 * @param definition 事务属性TransactionAttribute
	 * @param transaction 事务对象 DataSourceTransactionObject
	 * @param newTransaction 是否新建事务
	 * @param newSynchronization
	 * @param debug
	 * @param suspendedResources 挂起的对象
	 * @return
	 */
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				if (transaction != null) {
					//挂起当前事务，并返回之前存在的ConnectionHolder
	 ...
	 */
	/**
	 * 恢复之前被挂起的事务资源，恢复的操作主要就是重新绑定数据源和连接持有者
	 * 以及设置几个ThreadLocal的值
	 * @param transaction 当前的事务对象
	 * @param resourcesHolder 挂起的事务资源
	 * @throws TransactionException
	 */
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				//新建事务时，这个判断和回滚的时候是一致的，这个保证了即使切面执行了提交方法，但是没有新建事务
				//就不执行connection.commit();方法
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			try {
				//触发所有TransactionSynchronization的前置方法
				triggerBeforeCompletion(status);
				//存在保存点
				if (status.hasSavepoint()) {
					if (status.isDebug()) {
			...
			//触发所有TransactionSynchronization的后置方法
			triggerAfterCompletion(status, TransactionSynchronization.STATUS_ROLLED_BACK);
	...
	protected static class SuspendedResourcesHolder {
		...
		/**
		 * 创建一个挂起的资源持有者
		 * @param suspendedResources ConnectionHolder
		 * @param suspendedSynchronizations 同步器 empty
		 * @param name 事务名称
		 * @param readOnly
