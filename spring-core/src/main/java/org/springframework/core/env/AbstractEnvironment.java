...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	/**
	 * 默认的profiles，可以看到这里默认添加了default
	 */
	...
	protected Set<String> doGetActiveProfiles() {
		synchronized (this.activeProfiles) {
			if (this.activeProfiles.isEmpty()) {
				//从全局配置文件中加载 key为spring.profiles.active属性的值，注意这时候这里的全局配置文件
				//如果未加载其他配置文件的话，就是StandardEnvironment对象中加载的两个
				//System.getProperties()和System.getenv()
	...
	protected Set<String> doGetDefaultProfiles() {
		synchronized (this.defaultProfiles) {
			//未加载spring.profiles.default
			if (this.defaultProfiles.equals(getReservedDefaultProfiles())) {
				//获取环境变量中的spring.profiles.default
	...
	protected boolean isProfileActive(String profile) {
		...
		//存在于spring.profiles.active 或者spring.profiles.active为空并且存在于spring.profiles.default和profile为default
