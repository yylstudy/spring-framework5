...
public abstract class AbstractAspectJAdvice implements Advice, AspectJPrecedenceInformation, Serializable {
	...
	public static JoinPoint currentJoinPoint() {
		//获取方法执行器 ReflectiveMethodInvocation
		...
		if (jp == null) {
			//创建一个MethodInvocationProceedingJoinPoint
	 ...
	 */
	/**
	 * 参数名称数组，如果第一个参数是JoinPoint，那么该数组就是THIS_JOIN_POINT和argNames的并集
	 */
	@Nullable
	...
	public void setArgumentNamesFromStringArray(String... args) {
		...
		if (this.argumentNames != null) {
			//参数名称个数等于argNames的个数+1，那么就可能是存在JoinPoint或者ProceedJoinPoint等
			if (this.aspectJAdviceMethod.getParameterCount() == this.argumentNames.length + 1) {
				// May need to add implicit join point arg name...
				//第一个参数是JoinPoint，则增长参数数组
	...
	private void bindExplicitArguments(int numArgumentsLeftToBind) {
		...
		//排除第一个参数是JoinPoint的参数下标开始（也就是如果第一个参数类型是JoinPoint那么值为1，否则为0）
		int argumentIndexOffset = this.parameterTypes.length - numArgumentsLeftToBind;
		//排除第一个参数是JoinPoint，建立参数的映射关系
	 ...
	 */
	/**
	 * 获取JoinPoint对象 普通的JoinPoint的实现类是MethodInvocationProceedingJoinPoint
	 * @return
	 */
