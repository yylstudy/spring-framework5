...
public abstract class AbstractAdvisorAutoProxyCreator extends AbstractAutoProxyCreator {
	...
	/**
	 * AnnotationAwareAspectJAutoProxyCreator实现了BeanFactoryAware接口，所有在实现完这个后或执行setBeanFactory方法
	 * @param beanFactory
	 */
	@Override
	public void setBeanFactory(BeanFactory beanFactory) {
		...
		//初始化beanFactory（主要看AnnotationAwareAspectJAutoProxyCreator的实现）
	...
	protected List<Advisor> findEligibleAdvisors(Class<?> beanClass, String beanName) {
		//获取容器中所有的增强器，一个切面类中可以有多个Advisor
		//Advisor就是@After、@Before等对应的方法
		List<Advisor> candidateAdvisors = findCandidateAdvisors();
		//查找匹配的增强器
		List<Advisor> eligibleAdvisors = findAdvisorsThatCanApply(candidateAdvisors, beanClass, beanName);
		//子类添加增强器，这里是添加默认的ExposeInvocationInterceptor.ADVISOR
