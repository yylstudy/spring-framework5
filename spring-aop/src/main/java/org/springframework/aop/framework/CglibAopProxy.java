...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			//设置完整的代理接口（默认添加 SpringProxy、Advised、DecoratingProxy这三个接口）
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		if (exposeProxy) {
			targetInterceptor = (isStatic ?
					//创建 StaticUnadvisedExposedInterceptor
					new StaticUnadvisedExposedInterceptor(this.advised.getTargetSource().getTarget()) :
					new DynamicUnadvisedExposedInterceptor(this.advised.getTargetSource()));
		}
		else {
			targetInterceptor = (isStatic ?
					//创建 StaticUnadvisedInterceptor
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		/**
		 * 基于cglib的aop实现，注意这里并没有调用
		 * methodProxy.invokerSuper(proxy,args) 所以基于cglib的aop方法的内部调用也是无效的，因为此时方法内部调用的this
		 * 就是被代理对象，如果执行了methodProxy.invokerSuper(proxy,args)  方法内部调用的this应该是被代理对象
		 * 所以cglib动态代理依然能拦截方法内部调用
