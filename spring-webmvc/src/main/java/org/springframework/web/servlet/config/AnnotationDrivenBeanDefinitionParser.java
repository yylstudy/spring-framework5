...
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		//获取ContentNegotiationManagerFactoryBean的RuntimeBeanReference
		...
		//获取类型转换器，默认是DefaultFormattingConversionService
		...
		//创建一个RequestMappingHandlerAdapter
		...
		//创建ConversionServiceExposingInterceptor，这个拦截器的实现的主要作用就是
		//将类型转换器，默认是DefaultFormattingConversionService 添加进当前HttpServletRequest中
		RootBeanDefinition csInterceptorDef = new RootBeanDefinition(ConversionServiceExposingInterceptor.class);
		csInterceptorDef.setSource(source);
		csInterceptorDef.getConstructorArgumentValues().addIndexedArgumentValue(0, conversionService);
		//创建一个拦截器 MappedInterceptor
		RootBeanDefinition mappedInterceptorDef = new RootBeanDefinition(MappedInterceptor.class);
		mappedInterceptorDef.setSource(source);
		mappedInterceptorDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
		//拦截器的匹配地址为空，默认是拦截所有的请求
		mappedInterceptorDef.getConstructorArgumentValues().addIndexedArgumentValue(0, (Object) null);
		mappedInterceptorDef.getConstructorArgumentValues().addIndexedArgumentValue(1, csInterceptorDef);
		//注册MappedInterceptor
		String mappedInterceptorName = readerContext.registerWithGeneratedName(mappedInterceptorDef);
		//创建ExceptionHandlerExceptionResolver，这个应该是解析@ExceptionHandler注解的
	...
	/**
	 * 获取content-negotiation-manager属性
	 * @param element
	 * @param source
	 * @param parserContext
	 * @return
